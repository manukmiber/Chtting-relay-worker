    ...
    blocks: [
      ...
      },
      {
        list: [
          ...
          'The final data frame carries `usage`, and arrives whether or not `stream_options.include_usage` was sent. Detect it by the presence of `usage`, not by an empty `choices`: it usually has `choices: []`, but the last frame can carry both at once. Guard `choices[0]` rather than assume it.',
          '`id` is a UUIDv4 for this request, stable across every frame of one response. It is also the `X-Request-Id` header on every answer; an error carries neither.',
          'Each choice carries `native_finish_reason` beside `finish_reason`, always the same value. Read either.',
  ...
  return {
    ...
    blocks: [
      ...
      { code: JSON.stringify({ error: { message: 'model "gpt-9" not found', type: 'invalid_request_error', code: 'model_not_found' } }, null, 2), lang: 'json' },
      {
        ...
        rows: [
          ['400', 'none', 'Malformed JSON, or no `model` field.'],
          ['400', '`invalid_request`', 'A body this model will not accept. Its `type` reads `server_error`, unlike every other 4xx, so branch on the status and `code`.'],
          ['401', '`invalid_api_key`', 'Missing or unknown API key.'],
          ['403', '`invalid_api_key`', 'The key is disabled.'],
          ['403', '`model_forbidden`', 'The key is not entitled to that model.'],
          ['403', 'none', 'Type `forbidden`, message `blocked`: the caller\u2019s address is not served.'],
          ['404', '`model_not_found`', 'Unknown model id, or a path that does not exist.'],
          ['413', '`context_length_exceeded`', 'The prompt is over the model context length.'],
          ['413', '`too_large`', 'The request is too large for the model to take.'],
          ['413', '_no body_', 'Over the request body size limit. Refused before it can be read, so this one comes back as `text/plain`, not the JSON shape above \u2014 a client that parses every error body as JSON throws here.'],
          ...
          ['500', '`internal_error`', 'Something went wrong here before an answer existed. Rare, always worth reporting.'],
          ['502', '`model_unavailable`', 'The model could not answer. Retry with backoff; if it persists, get in touch with the model id and roughly when it started \u2014 an error carries no request id.'],
